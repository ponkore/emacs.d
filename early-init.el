...
;;
;; 注意: 以前は tr-ime / w32-ime だけがこの自動有効化に依存していたが、
;; init.el 側で straight による明示宣言に切り替えたのでここで無効化できる。
;; もし Windows の IME に問題が出たら、まずこの行を t に戻して切り分けること
;; (elpa/ は削除していないので、それだけで元の状態に戻る)。

...
`emacs-config-refactoring-plan.md` の記載を以下の通り訂正する。
...
## 9. 起動時間（0-3 / 3-7、2026-09-02 追記）

フェーズ 0 当時に `emacs-init-time` は計測されておらず、この記録にも数値が無い。
旧構成（`89f053e^`、Org タングル方式）の再現には package.el と straight の
両方でパッケージを一から取得する必要があり、しかも停止済みの ELPA
（marmalade / orgmode.org）を参照するため起動するかも怪しい。割に合わないため
実測は行わない。

**CLAUDE.md に残る「約 1250ms」を暫定の基準とする。**
ただしこれはフェーズ 3 完了後にバイトコンパイルの有無を比べたときの値であって、
フェーズ 0 の値ではない。

### 2026-09-02 の実測（現行構成 = `e2cc8b5`）

GUI 起動（`emacs --eval '(progn ... (emacs-init-time) ... (kill-emacs))'`）を
6 回。1 回目はディスクキャッシュが温まっていないため参考値。
...
2〜6 回目で **中央値 0.967 秒 / 平均 0.981 秒**。

暫定基準の 1250ms に対して悪化は無く、むしろ約 280ms 速い。3-7 の完了条件
「悪化していない」は満たす。

環境: Emacs 31.1 / Windows 11 / モニタ 3 面（プライマリ 2560x1440）。
`user-lisp/` はバイトコンパイルしていない（JUST-ACTIVATE = t）。

...
2. 見出しと行中のコード。行中のコードは `` ` `` を落として色だけ残す。
...
```
│ 水位法（`my:claude--cap- │ 全列に共通の上限 `L` を  │   ← ここで分かれると
│ widths`）                │ 置き、`Σ min(w, L)` が  │
```

上の行の `` `my:claude--cap- `` は行内で閉じないので、旧順序では
**次の `` ` `` すなわち `` `）` の手前 `` と組になり、罫線と詰め物まで
色が付いていた**。列ごとに face の付いた文字を拾って比べると差がはっきり出る。

```
基準（折り返さない）: "`my:claude--cap-widths``my:claude--kinsoku-point`"
旧順序（w=60）:       "`my:claude--cap- `）                `my:claude--kinsok `）　　　"
新順序（w=60）:       "`my:claude--cap-widths``my:claude--kinsoku-point`"  ← 基準と一致
```

**バッファの順に拾って比べてはいけない。** 折り返すと左右のセルが
行方向に交互に並ぶので、列をまたいで混ざり、正しく動いていても
不一致に見える（最初この誤った判定で「直っていない」と読み違えた）。

順序を変えた副産物として、`my:claude--protect` が張った read-only の
上に `put-text-property` することも無くなった。旧順序は
`my:claude--fontify-markdown` の `inhibit-read-only` に暗黙に
依存していた。
...
#### 行と行の間にも罫線を引く（2026-09-17）

見出しと本体の境目だけでなく、本体の各行の間にも `├┼┤` を挟む。

```
┌────────────────┬──────────────────────────────┐
│ 手順                   │ 結果                         │
├────────────────┼──────────────────────────────┤
│ 1. main へマージ → push │ `15eb8b02`（マージコミット） │
├────────────────┼──────────────────────────────┤   ← これ
│ 2. ブランチ削除        │ ローカル削除。リモートに無し │
└────────────────┴──────────────────────────────┘
```

`mapconcat` の区切りに罫線を渡すだけ。**本体が 0 行のとき（見出しと
区切り行しか無い表）に区切りをぶら下げない**よう、見出しと本体の
両方がある場合だけ境目を出す（`└` の直前に `├` が来ると崩れて見える）。

### 幅を超えたらセルの中で折り返す（2026-09-17）

`my:claude-table-max-width`（既定 nil = ウィンドウ幅）。**収まるうちは
何もしない**ので、短い表の見た目は変わらない。

#### 幅は `window-max-chars-per-line` で測る

`window-body-width` では足りない。行番号の欄や継続記号のぶんを引いて
くれるのはこちらだけで、`display-line-numbers-mode` が有効なバッファでは
実際に使える幅が数桁狭い（CLAUDE.md「桁は自分で数えないこと」）。
ウィンドウに出ていなければ `my:claude--table-fallback-width`（100）。

#### 【重要】縦罫 `│` も 1 文字 rw 桁ある

予算から先に引くオーバーヘッドは `(ncol+1)*rw + 2*ncol`。**`─` だけを
勘定して `│` を 1 桁と数えると、`ncol+1` 本ぶん足りず、組み上がった表が
毎回ウィンドウ幅を超える。**

```
上限 40 に対して 42 桁 |│ 変更           │ 内容               │|
```

**batch では rw が 1 なので、この誤りはたまたま正しい値になる。**
`3*ncol+1` と `(ncol+1)*rw+2*ncol` は rw=1 のとき一致するため、
batch のプローブは全ケース「収まっている」と答える。GUI で
`string-pixel-width` を測って初めて分かった。

刻みの調整（`w+2` を rw の倍数に広げる）で列ごとに最大 `rw-1` 桁
広がるぶんも、**先に**引いておく。あとから広げるとはみ出す。

#### 列幅は水位法で配分する

全列に共通の上限 `L` を置き、`Σ min(w, L)` が予算に収まる最大の `L` を
二分探索する（`my:claude--cap-widths`）。**狭い列は削らない。**
広い列だけが切られるので、短い見出しの列は自然幅のまま残る。

```
┌────────────────┬──────────────────────────────────┐
│ 変更           │ 内容                             │
├────────────────┼──────────────────────────────────┤
│ 行の分離       │ `JSON/SqlDefinition.json`（本体＝ │
│                │ サブファイル参照のみ）と         │
├────────────────┼──────────────────────────────────┤
│ 理由の書き換え │ 「サブシステム単位で複数機能の定義 │
│                │ が同居し、こちらと ESC の双方が同 │
└────────────────┴──────────────────────────────────┘
```
...
`my:claude--wrap-cell`。日本語には単語境界が無いので桁で切るのが基本で、
そこへ 2 つの例外を足す。

| | |
|---|---|
| 禁則（`my:claude--kinsoku-point`） | 行頭に `。」` 等、行末に `「（` 等が来るときは追い出す。**戻すのは 2 文字まで**。`……」。` のように続くと際限なく戻り、その行がほとんど空になる |
| ASCII の語（`my:claude--word-break-point`） | 語の途中で切れるなら語頭まで戻す。**行の後半まで戻せるときだけ**。長い URL のように語そのものが 1 行より長いと、行頭まで戻って何も載らないまま無限に折り返す |

`kinsoku.el` は使わない。あれは `fill` の経路から呼ばれる前提で
バッファの point を動かす作りなので、文字列を切る用途に合わない。

**テキストプロパティは `substring` が持ち越す**ので、`**強調**` や
行中のコードの face は折り返した先にも残る。ただしそれは、セルに
face が載っていればの話で、そこに 2 つ仕掛けが要る。

#### 【重要】セルは `font-lock-face` だけを持つ文字列として取り出す

`my:claude--line-face-string`。どちらの極端もまずい。
...
1 文字ずつ `aref` して `(apply #'string …)` で組み直すと**テキスト
プロパティが落ちる**。区切りを探すところだけ文字のループで行い、
切り出しは範囲で行う。`\|` を `|` に戻した 1 文字だけは face が落ちるが、
セルの中の `|` は罫線と紛れるので元から装飾の対象にならない。

#### 【重要】幅は組んだ時点のもので固定される

罫線に組み直した時点で元のパイプ表は捨てているので、**あとから
ウィンドウを広げても組み直さない**（`my:claude-toggle-maximize`、
左右分割）。追従させるには原文をテキストプロパティに退避して
`window-size-change-functions` から組み直すことになるが、確定領域を
書き換えるたびに `my:claude--protect` を張り直す必要があり、割に合わない。

### 【重要】`**強調**` は記号ごと消す（2026-09-17）

`my:claude-render-emphasis`（既定 t）。`**` を残したまま太字にすると
**記号のぶんだけ桁がずれる**。表のセルでは実害が出る。列幅は
`string-width` で決めるので、見えている記号 4 桁がそのまま列幅に乗る。

そこで**記号を消して face だけを残す**（`my:claude--render-emphasis`）。
表より前に走るので、セルの `**` は罫線に組む時点でもう消えており、
列幅に乗らない。

#### 【重要】face は重ねる。上書きしない

確定した会話には挿入時に `my:claude-assistant-face` などが載っている。
`put-text-property` で単に差し替えると**発言の色が落ちる**。
`my:claude--add-face` が既存の値を `my:claude--face-list` でリストにして
先頭に積む（先に書いた属性が勝つ）。

`my:claude-bold-face` が色を持てるのはこの順序のおかげで、**載る先が
本文だけ**だからでもある。`my:claude--fontify-markdown` を通るのは
assistant のテキストブロックだけで、サブエージェントの発言もツールの
結果も通らない。唯一重なりうる見出しでは face を載せない（下記）。

#### 拾わないもの

正規表現は `**` の内側を「空でなく、両端が空白でなく、`*` を含まない」に
限る（markdown の規則）。
...
| `**a**` / `**a b**` | 太字にする |
| `** a **` | **拾わない**（両端の空白） |
| `2*3` / `* glob` | **拾わない**（`*` 1 つの強調は見ない） |
| `` `**a**` `` | **拾わない**（行中のコードの中） |
| ``` の中 | **拾わない** |

`*` 1 つの強調を拾わないのは、掛け算やグロブに当たるうえ、claude の
出力では強調に使われないため。

### 行中のコードも `` ` `` ごと消す（2026-09-18）

`my:claude--render-inline-code`。**理由は `**強調**` と同じ**で、色が
付いていれば範囲は分かるのに、記号 2 桁がそのまま表の列幅に乗る。

**`my:claude--render-emphasis` より先に呼ぶ。** あちらは
`my:claude-inline-code-face` を見てコードの中の `**` を避けるので、
face がまだ載っていないと `` `**a**` `` の `**` まで消してしまう。

**見出しの中では記号だけ落とし、face は載せない。** 見出しは行ごと
...
端末の claude（TUI）の見え方に合わせた。

| face | dark | |
|---|---|---|
| `my:claude-assistant-face` | `#b8b8b8` | 本文。`default`（`#ffffff`）より 1 段暗い |
| `my:claude-bold-face` | `#ffffff` + bold | 太さだけでなく**明るさ**でも差が付く |
| `my:claude-inline-code-face` | `font-lock-constant-face`（`#00bcff`） | 変えていない |

`shadow`（`#989898`）より明るくしてあるのは、ツールの結果
（`my:claude-tool-result-face`）と補足（`my:claude-meta-face`）が
そちらだから。本文が同じ灰色になると区別が付かない。

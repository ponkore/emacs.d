...
## 2026-09-04 追記: 案 C を実装したあとの結論

案 C（ConPTY プロキシ + 端末エミュレータ）は**動くところまで作った**。
`ptyd/` + `user-lisp/my-pty.el`、表示は eat。claude の通常モードの TUI が
Emacs のバッファに出て、日本語の入出力もできる。

**それでも日常的に使う気にはならない、というのが実際に触ってみた結論。**
理由は 3 つで、どれも PTY の問題ではなかった。

### 1. 文字幅はフォントの問題で、Emacs では制御しきれない

桁を数えているのは claude と conhost で、どちらも East Asian Ambiguous を
**幅 1** として組む。ところが Emacs の描画は `char-width-table` ではなく
**フォントの送り幅**で決まる。日本語フォントは `─` `○` `①` を全角で
描くので、`char-width` をどう変えても揃わない。

しかも **`char-width-table` もフォントもバッファ単位に変えられない。**
`set-fontset-font` はこの設定では効かず（`nil` にも `t` にも入れ、
`clear-face-cache` まで呼んでも `font-at` が元のフォントを返す）、
実際に効くのは `set-face-attribute 'default` だけ。つまり
**端末のために Emacs 全体のフォントを差し替えるしかない。**

編集中のバッファまでフォントが変わるのは実用上つらいので、
`my:pty-console-font` の既定は nil（切り替えない）にしてある。

| フォント | `a` | `あ` | `─` | `①` | `★` |
|---|---|---|---|---|---|
| HackGen（通常） | 8 | 16 | 16 | 16 | 16 |
| HackGen Console NF | 8 | 16 | 8 | 16 | 16 |
| Consolas | 8 | 16 | 8 | 8 | 16 |

`★` `※` はどのフォントでも全角なので、そもそも揃えられない。

なお **WezTerm では HackGen Console NF で `①` が半角で表示される。**
端末エミュレータは自前で桁を持つので、フォントの送り幅と桁数を
切り離せる。Emacs のバッファ表示ではそれができない。

### 2. eat の入力レスポンスが実用に耐えない

文字入力とカーソル移動の反応が明らかに遅い。純 elisp の端末エミュレータ
としては仕方のないところ。

### 3. そもそも案 A で足りている

案 A（stream-json）は端末を経由しないので、上の問題が **1 つも起きない**。

### 残したもの

`ptyd/` と `my-pty.el` は消さずに残す。汎用の PTY プロキシとしては
正しく動いており、他の対話 CLI に使う道はある。`M-x my:claude-term`。

### 案 A に戻るときの持ち帰り

claude が出す**表**は案 A でも問題になりうる。ただし案 A で来るのは
markdown なので、区切りは ASCII の `|` と `-`。全角文字だけなら桁は合う。
崩れるのは `★ ※ ①` のような ambiguous を**セルの中に含むとき**だけ。

必要なら、受け取った markdown の表を Emacs 側で組み直す（Emacs の
`string-width` で測り直してパディングし直す）のが筋。端末と違って
案 A では**表示の主導権がこちらにある**ので、これは実現できる。

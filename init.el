...
;;
;; ここでバイトコンパイルすると、パッケージ由来のマクロを use-package の
;; :init / :config で使っている箇所が壊れる。この時点ではまだ下の require 列を
;; 通っていないため、マクロが未定義で関数呼び出しとしてコンパイルされる。
...
;;
;; require 列の後まで遅らせればマクロは正しく展開されるが (バイトコンパイラは
;; autoload されたマクロを自分でロードする)、それでもコンパイルしない。
;; 2026-09-05 の実測で .elc あり 0.993s / 無し 0.995s と差が無く
;; (このビルドは native-comp が使えないので byte-code 止まり)、
;; 一方で「そのセッションでロードされていたパッケージ次第で .elc の中身が
;; 変わる」「load-prefer-newer が nil なので古い .elc が .el に勝つ」という
;; 不確実さだけが増えるため。詳細は CLAUDE.md の user-lisp/ の節。
...
(require 'my-htnblog)   ; はてなブログへ投稿する
